...
export default function Home() {
  ...
  }, [setAwaitingIndexing, setCurrentJobId]);
  ...
  return (
    <div className="welcome-content flex flex-col h-screen min-h-0 gap-2">
      <div className="flex flex-row items-center flex-none min-h-[55px] bg-zinc-800/60 pl-4 ">
        <Searchbar
          ...
          placeholder="Search for files, images, or videos…"
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              e.preventDefault();
              handleSearch();
      ...
      {hasInteracted ? (
        <div
          className={cn("flex flex-1 min-h-0", "bg-zinc-800/75", "px-4")}
        >
          {isLoading ? (
        ...
        >
          ...
          <div className="text-sm text-zinc-500">Start typing to search...</div>
        </div>
      )}

      <div className="flex-none w-full shrink-0 border-t border-zinc-700/50 bg-zinc-900/35 px-3 py-2">
        <Footer />
      </div>
    </div>
  );
